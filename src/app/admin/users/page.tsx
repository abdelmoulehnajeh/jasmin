      ...
      <div className="space-y-6">
        ...
        <div>
          <h1 className="text-4xl font-black text-white mb-2">
            GESTION <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold-500 to-burgundy-500">UTILISATEURS</span>
          </h1>
          <p className="text-gray-400">Gérer tous les utilisateurs</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: 'Total Users', count: users.length, color: 'from-blue-600 to-cyan-600', icon: '👥' },
            { label: 'Admins', count: users.filter(u => u.role === 'ADMIN').length, color: 'from-gold-600 to-burgundy-600', icon: '👑' },
            { label: 'Clients', count: users.filter(u => u.role === 'USER').length, color: 'from-green-600 to-emerald-600', icon: '👤' },
          ].map((stat, i) => (
            ...
            <div key={user.id} className="bg-gray-900 border-2 border-gold-500/20 hover:border-gold-500 rounded-2xl p-6 transition-all">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                  user.role === 'ADMIN' ? 'bg-gold-500/20 text-gold-500' : 'bg-blue-500/20 text-blue-500'
                }`}>
                  {user.role}
                </span>
              </div>
        ...
        {filteredUsers.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-2xl text-gray-400">Aucun utilisateur trouvé</p>
          </div>
        )}
      </div>
    </AdminLayout>
